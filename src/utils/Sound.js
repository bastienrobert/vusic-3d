...
export default class Sound {
  ...
  get time() {
    return this.isPlaying
      ? this.ctx.currentTime - this._startTime
  ...
  onUpdate() {
    ...
    for (let i in this._sections) {
      if (this._sections[i].condition()) this._sections[i].callback.call(this)
    }
...
class Debug {
  ...
  draw() {
    ...
    for (let i = 0, len = kickLength; i < len; i++) {
      kick = kicks[i]
      if (kick.isOn) {
        kickFrequencyStart = kick.frequency.length
          ? kick.frequency[0]
          : kick.frequency
        kickFrequencyLength = kick.frequency.length
          ? kick.frequency[1] - kick.frequency[0] + 1
          : 1
        this.ctx.beginPath()
        this.ctx.rect(
          kickFrequencyStart * spectrumWidth,
          spectrumHeight - spectrumHeight * (kick.threshold / 256),
          kickFrequencyLength * spectrumWidth - spectrumWidth * 0.5,
          2
        )
        this.ctx.rect(
          kickFrequencyStart * spectrumWidth,
          spectrumHeight - spectrumHeight * (kick.currentThreshold / 256),
          kickFrequencyLength * spectrumWidth - spectrumWidth * 0.5,
          5
        )
    ...
    if (sectionLabels.length > 0)
      sectionLabels = sectionLabels.substr(0, sectionLabels.length - 3)
